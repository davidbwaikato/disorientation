        ...
        this.asGeom = function () {
            ...
            for ( var i = 0; i < paths.length; i ++ ) {
                    ...
                    for ( var j = 0; j < shapes.length; j ++ ) {
                            ...
                            var shape = shapes[ j ];
                            var mesh = createMesh(new THREE.ExtrudeGeometry(shape, options));
                            group.add( mesh );
//scene.add(createMesh(new THREE.ShapeGeometry(shape)));if (j==0){console.log("SECOND SHAPE");console.log(mesh)}
                    }
            ...
            var locationMarker = new THREE.Mesh( new THREE.BoxGeometry( 1, 1, 50 ), new THREE.MeshBasicMaterial( {color: 0xff0000} ) );
            scene.add(locationMarker)
            navigator.geolocation.getCurrentPosition(function(position) {
                console.log(position.coords.latitude, position.coords.longitude);
                //o.position.set(cx + (position.coords.longitude - 175.319027)*0.08797305976967*1000000, cy + (position.coords.latitude - -37.788739)*0.11131949052531577*1000000, 0)
                setAtCoords(locationMarker, position.coords.longitude, position.coords.latitude);
            ...
            //var e_2 = new Floorplan('./data/E-2.json').relativeTo(f_2, -348652.9613, -50762.3970).position(-2747303.3611, 856267.4823).angle(0).level(2).build(m)
            
            //drawFloor('./data/G-B.json', 142595.0645, -207485.7364, -1)
            //drawFloor('./data/G-G.json', 529327.655, -34219.737, 0)
            /*drawFloor('./data/G-1.json', 38069.4879, 20002.2851, 1)
            drawFloor('./data/G-2.json', 1240906.4223, 1581547.1668, 2)
            drawFloor('./data/G-3.json', -538076.7921, 491647.7904, 3)
            */
            //drawFloor('./data/FG.json', 349210176.381, 598963531.956, 0)
            /*drawFloor('./data/FG.json', 349310176.381, 598963531.956, 1)
            drawFloor('./data/FG.json', 349410176.381, 598963531.956, 2)
            drawFloor('./data/FG.json', 349510176.381, 598963531.956, 3)
            drawFloor('./data/FG.json', 349610176.381, 598963531.956, 4)
            */
...
scene = new THREE.Scene();
    ...
    loader.load(
        ...
        function ( xhr ) {
            ...
            //if (xhr.loaded / xhr.total == 1) init();
...
class Floorplan {
    ...
    drawPolylines(color) {
        ...
        var height = this.floorLevel * 4 - 4;
        var group = new THREE.Group();
        var a=0;
